...
# wavelength = list(np.arange(3800, 9300, 200))
...
for file in os.listdir(directory):
    ...
    if filename.endswith(".fits"): 
        ...
        for wavl_ind in range(0, len(wavelength)):
            ...
            #row.append(calc_snr_at_wav(inputSpectrum,wavelength[wavl_ind],Fiber,calculate_at_center=True)[1]) #SNR
            ...
            # print("flux sqrt = snr : {}".format(np.sqrt(calc_snr_at_wav(inputSpectrum,wavelength[wavl_ind],Fiber,calculate_at_center=True)[1])))
...
# ax1.set_ylabel("SNR") 
# plt.savefig("Eclipse_Figures/ext_coeff_eclipse.png")
...
# plt.savefig("Eclipse_Figures/flux_coeff_eclipse.png")
# plt.clf()
...
plt.savefig("Eclipse_Figures/intensity_coeff_eclipse_new.png")
plt.clf()
...
plt.savefig("Eclipse_Figures/coeff_eclipse.png")
plt.clf()
...
for i, value in enumerate(wavelength):
    ...
    ax1.scatter(df['datetime'], file_data[intensity_list[i]][()]/max(file_data[intensity_list[i]][()]), label = wavelength[i], color=color)
    ax1.plot(df['datetime'], df[wavelength[i]]/max(df[wavelength[i]]), color=color)
plt.legend()
ax1.set_xlabel("hour on 10/14")
ax1.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
ax1.set_ylabel("relative flux") 
plt.savefig("Eclipse_Figures/flux_comp.png")
plt.clf()
