...
axs[0].scatter(time, raw_rv + vb, color = 'r', marker = "x", s = 5, label = "Reiners corrected RVs")
offset1 = (raw_rv[-1] + vb[-1] - RV_list_no_cb[-1])
axs[0].scatter(time, RV_list_no_cb + offset1, color = 'k', s = 5, label = "Model - No CB")
offset2 = (raw_rv[-1] + vb[-1] - RV_list_cb[-1])
...
axs[0].legend()
#residuals
axs[1].scatter(time, (raw_rv + vb-offset1) - RV_list_no_cb, color = 'k', marker = "x", s = 1)  
axs[1].scatter(time, (raw_rv + vb-offset2) - RV_list_cb, color = 'grey', marker = "x", s = 1)  
...
fig = plt.figure()
ax1 = fig.add_subplot()
ax1.scatter(time, intensity_list)  
