...
fig, axs = plt.subplots(2, figsize=(7, 8), sharex=true, sharey=false,
...
for (i, value) in enumerate(vacwav)
    ...
    axs[1].scatter(airmass, grass_normalized, color=color, s = 1)
    axs[1].plot(airmass, neid_normalized, color=color)
    axs[2].scatter(airmass, comp_arr, color=color, s = 1) 
end
axs[1].set_xlabel("Airmass", fontsize=12)
axs[1].set_ylabel("Relative Intensity", fontsize=12) 
axs[2].set_xlabel("Airmass", fontsize=12)
axs[2].set_ylabel("Residuals", fontsize=12) 
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
axs[1].set_title("Limb Darkening Only")
plt.tick_params(axis="x", which="both", top=true, labeltop=false)
plt.savefig("no_ext.png")
plt.clf()
...
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
axs[1].set_title("Limb Darkening + Extinction")
plt.tick_params(axis="x", which="both", top=true, labeltop=false)
plt.savefig("ext.png")
plt.clf()