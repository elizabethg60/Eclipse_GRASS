...
# read in data
# GRASS
grass_data = h5py.File("data/boulder_rv_on.jld2", "r")
...
def plot_line1(time_stamps_data, rv_obs, model, model_label, GRASS, GRASS_label, LD, save, rv_error_GRASS_no_cb_array):
...
def plot_line2(time_stamps_data, rv_obs, GRASS, GRASS_label, LD, save, rv_error_GRASS_no_cb_array):
    fig, axs = plt.subplots(2, sharex=True, sharey=False, gridspec_kw={'hspace': 0, 'height_ratios': [3, 1]})
    axs[0].scatter(time_stamps_data, rv_obs, color = 'k', marker = "x", s = 18, label = "Real RVs")
    axs[0].plot(time_stamps_data, GRASS, color = 'b', linewidth = 2, label = GRASS_label)
    axs[0].xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
...
for i in range(0,len(LD_arr)):
    ...
    # rm curve 
    plot_line1(time_stamps_data, rv_obs, RV_list_no_cb_array, "Weighted RVs - No CB", GRASS_no_cb_array, "Line RVs - No Var", LD_arr[i], "no_granulation", rv_error_GRASS_no_cb_array)
    plot_line2(time_stamps_data, rv_obs, GRASS_rv_array, "Line RVs - Var", LD_arr[i], "granulation", rv_error_GRASS_cb_array)