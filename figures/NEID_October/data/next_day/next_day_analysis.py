...
#110 points, binned by 5 so 22 bins each with 5 points
...
def jld2_read(jld2_file, variable, vb, index):
    array = jld2_file[variable[index]][()][0:-1]
...
def plot_3(projected, grass_cb, grass_no_cb, grass_cb_err, 
    ...
    for i in range(0,len(lines)):
        ...
        line_err.append(np.mean(line_file[line_rv_err[i]][()][0:-1]))

        lines_arr.append(lines[i])

        projected_arr_pipe.append(rms(bin_array(projected_i,5)))
        grass_cb_arr_pipe.append(rms(bin_array(grass_cb_i,5)))
        grass_no_cb_arr_pipe.append(rms(bin_array(grass_no_cb_i,5)))
        line_rv_pipe.append(rms(bin_array(line_i,5)))

        residuals.append(rms(bin_array(line_i-grass_no_cb_i,5)))

        if i == 4 or i == 10 or i == 14 or i == 15: 
            continue
