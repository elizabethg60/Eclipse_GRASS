...
# NEID
grass_data = h5py.File("/storage/home/efg5335/work/Eclipse_GRASS/figures/NEID_October/HD/data/neid_all_lines_rv_regular_HD.jld2", "r")
GRASS_rv  = grass_data["rv"][()]
file = h5py.File("/storage/home/efg5335/work/Eclipse_GRASS/figures/NEID_October/HD/data/neid_october_N_50_HD.jld2", "r")
...
rms_grass_no_cb_grass = round(np.sqrt((np.nansum((line_rv_array - GRASS_rv_array)**2))/len(line_rv_array - GRASS_rv_array)),2)
rms_grass_no_cb_model = round(np.sqrt((np.nansum((line_rv_array - RV_list_no_cb_array_neid)**2))/len(line_rv_array - RV_list_no_cb_array_neid)),2)
axs[0, 0].plot(UTC_time, RV_list_no_cb_array_neid, color = 'r', linewidth = 2, label = "Projected RVs ({} m/s)".format(round(rms_grass_no_cb_model,2)))
axs[0, 0].plot(UTC_time, GRASS_rv_array, color = 'b', linewidth = 2, label = "GRASS ({} m/s)".format(round(rms_grass_no_cb_grass,2)))
...
rms_grass_no_cb_model = round(np.sqrt((np.nansum((rv_obs_LHR - RV_list_no_cb_array)**2))/len(rv_obs_LHR - RV_list_no_cb_array)),2)
axs[0, 1].plot(time_stamps_data, RV_list_no_cb_array, color = 'r', linewidth = 2, label = "Projected RVs ({} m/s)".format(round(rms_grass_no_cb_model,2)))
