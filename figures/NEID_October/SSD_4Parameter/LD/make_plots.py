...
#these projected models are for N=197, sub=40x40, no extinction (LD only), with sunspots 
    # (only flux effect for model 1 & 3 but for model 2 also GRASS CB suppression), no MF/CB (model 2 has GRASS CB)
projected_SSD_4parameter = h5py.File("data/projected_SSD_4parameter_gpu.jld2", "r")
projected_RV_SSD_4parameter = projected_SSD_4parameter["RV_list_no_cb"][()]

grass_on_SSD_4parameter = h5py.File("data/neid_all_lines_rv_on_SSD_4parameter_gpu.jld2", "r")
grass_on_RV_SSD_4parameter = grass_on_SSD_4parameter["rv_asymmetric_lsf"][()]

grass_off_SSD_4parameter = h5py.File("data/neid_all_lines_rv_off_SSD_4parameter_gpu.jld2", "r")
...
for i in range(0,len(line_list)):    
    ...
    rms = round(np.sqrt((np.nansum((line_rv_array - grass_on_SSD_4parameter_array)**2))/len(line_rv_array)),2)
    axs[0].plot(UTC_time, grass_on_SSD_4parameter_array, color = 'r', linestyle = "--", linewidth = 2, label = "Model Two: {} m/s".format(rms))
