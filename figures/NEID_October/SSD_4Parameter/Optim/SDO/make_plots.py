...
#these models are for N=197, sub=40x40, with extinction, with sunspots 
    # with and without CB optimized polynomial from projected RVs (flux & CB effect),
    # with SDO for MF/supergranulation 
projected_SSD_4parameter = h5py.File("data/projected_SSD_4parameter_gpu_ext_SDO_optim.jld2", "r")
projected_RV_SSD_4parameter = projected_SSD_4parameter["RV_list_no_cb"][()]

projected_SSD_4parameter_CB = h5py.File("data/projected_SSD_4parameter_gpu_ext_SDO_CB_optim.jld2", "r")
projected_RV_SSD_4parameter_CB = projected_SSD_4parameter_CB["RV_list_no_cb"][()] #0.4749330977888918, -1.9229054861855905, -3.6488879274922663
...
for i in range(19,20):#len(line_list)):    
    ...
    projected_RV_SSD_4parameter_array_CB = jld2_read(projected_SSD_4parameter_CB, projected_RV_SSD_4parameter_CB, vb, i)
    ...
    rms = round(np.sqrt((np.nansum((line_rv_array - projected_RV_SSD_4parameter_array_CB)**2))/len(line_rv_array)),2)
    axs[0].plot(UTC_time, projected_RV_SSD_4parameter_array_CB, color = 'g', linestyle = "--", linewidth = 2, label = "Model One: {} m/s".format(rms))
    ...
    print(round(np.sqrt((np.nansum((bin_array(line_rv_array,4) - bin_array(projected_RV_SSD_4parameter_array_CB,4))**2))/len(bin_array(line_rv_array,4))),2))
    ...
    axs[1].scatter(UTC_time, line_rv_array - projected_RV_SSD_4parameter_array_CB, color = 'g', marker = "x", s = 8) 
