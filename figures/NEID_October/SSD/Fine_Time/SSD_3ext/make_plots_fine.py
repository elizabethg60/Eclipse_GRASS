...
grass_data_no_cb = h5py.File("data/neid_all_lines_rv_off_SSD_gpu_fine_spectra_ext_1.jld2", "r")
lines_no_cb = grass_data_no_cb["name"][()]
GRASS_no_cb  = grass_data_no_cb["rv"][()]
grass_data_no_cb2 = h5py.File("data/neid_all_lines_rv_off_SSD_gpu_fine_spectra_ext_2.jld2", "r")
GRASS_no_cb2  = grass_data_no_cb2["rv"][()]
...
def plot_line(UTC_time, rv_obs, line_rv_array, GRASS, GRASS_label, path, save):
    ...
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.savefig("{}/{}.png".format(path,save))
    plt.clf()

def plot_pipe(UTC_time, rv_obs, line_rv_array, GRASS, GRASS_label, path, save):
    fig, axs = plt.subplots(2, sharex=True, sharey=False, gridspec_kw={'hspace': 0, 'height_ratios': [3, 1]})
    axs[0].scatter(UTC_time, rv_obs, color = 'y', marker = "x", s = 18, label = "NEID RVs")
    # axs[0].scatter(UTC_time, line_rv_array, color = 'k', marker = "x", s = 18, label = "NEID line RVs") 
    axs[0].plot(UTC_time, GRASS, color = 'b', linewidth = 2, label = GRASS_label)
    ...
    # axs[0].set_title("Fe I 5434 \u212B")
    # residuals
    axs[1].scatter(UTC_time, rv_obs - GRASS, color = 'b', marker = "x", s = 3)  
    axs[1].xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
    axs[1].set_xlabel("Time (UTC)", fontsize=12)
    axs[1].set_ylabel("Residuals", fontsize=12) 
    plt.xticks(fontsize=12)
...
for i in range(0,11):
    ...
    plot_pipe(UTC_time, rv_obs, line_rv_array, GRASS_no_cb_array, "GRASS Model", "No_Granulation", lines_no_cb[i])

for i in range(11,22):
    line_rv_array = jld2_read(line_data, line_rv, vb, i)
    GRASS_no_cb_array = jld2_read_fine(grass_data_no_cb2, GRASS_no_cb2, vb, i)
