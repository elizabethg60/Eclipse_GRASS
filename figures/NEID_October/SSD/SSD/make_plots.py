...
for i in range(0,1):#len(lines)):
    ...
    # rm curve 
    # plot_line(UTC_time, rv_obs, line_rv_array, RV_list_no_cb_array, "Weighted RVs - No CB", GRASS_no_cb_array, "Line RVs - No Var", "No_Granulation/LinebyLine", lines_no_cb[i], rv_error_line_array, rv_error_line_array)
    ...
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.savefig("No_Granulation/rm_and_residuals_{}_new.png".format(lines_no_cb[i]))
    plt.clf()
