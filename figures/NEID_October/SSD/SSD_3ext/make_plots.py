...
# read in data
# GRASS
grass_data = h5py.File("data/neid_all_lines_rv_regular_SSD_3ext.jld2", "r")
lines = grass_data["name"][()]
GRASS_rv  = grass_data["rv_lsf"][()]
rv_error_GRASS_cb  = grass_data["rv_error_lsf"][()]
grass_data_no_cb = h5py.File("data/neid_all_lines_rv_off_SSD_3ext.jld2", "r")
lines_no_cb = grass_data_no_cb["name"][()]
GRASS_no_cb  = grass_data_no_cb["rv_lsf"][()]
rv_error_GRASS_no_cb = grass_data_no_cb["rv_error_lsf"][()]
# model
file = h5py.File("data/neid_october_N_50_SSD_3ext.jld2", "r")
...
def plot_line(UTC_time, rv_obs, line_rv_array, model, model_label, GRASS, GRASS_label, path, save, rv_error_GRASS_no_cb_array, rv_error_line_array):
    ...
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.savefig("{}/rm_and_residuals_{}_lsf.png".format(path, save))
    plt.clf()
